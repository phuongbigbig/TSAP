...
- **Upload CSV / TSV** — the delimiter and layout are auto-detected. Two layouts are accepted: **wide** (first column `Time`, then one column per measurement; group replicates with shared `_R1/_R2…` names, or `_S1/_S2…` to mark the same subjects over time, which auto-enables repeated-measures mode) and **long/tidy** (columns `time`, `series`, `value`, optional `subject`). Use **Download template** to get a ready-made CSV of the current table.
